...
export default function PScoreChart({ scores, n, targetRank, targetP }) {
    ...
    <div style={{
      ...
      <ResponsiveContainer width="100%" height={260}>
        <LineChart data={data} margin={{ top: 8, right: 40, left: 0, bottom: 0 }}>
          ...
          {/* ランク境界線（標準） */}
          <ReferenceLine y={RANK_THRESHOLDS.C} stroke={RANK_COLORS.C} strokeDasharray="6 3"
            label={{ value: 'C', position: 'right', fill: RANK_COLORS.C, fontWeight: 'bold' }} />
          <ReferenceLine y={RANK_THRESHOLDS.B} stroke={RANK_COLORS.B} strokeDasharray="6 3"
            label={{ value: 'B', position: 'right', fill: RANK_COLORS.B, fontWeight: 'bold' }} />
          <ReferenceLine y={RANK_THRESHOLDS.A} stroke={RANK_COLORS.A} strokeDasharray="6 3"
            label={{ value: 'A', position: 'right', fill: RANK_COLORS.A, fontWeight: 'bold' }} />
          {/* 目標P点ライン（カスタム閾値の場合のみ太実線で表示） */}
          {targetP && ![RANK_THRESHOLDS.A, RANK_THRESHOLDS.B, RANK_THRESHOLDS.C].includes(targetP) && (
            <ReferenceLine y={targetP} stroke="#D32F2F" strokeWidth={2}
              label={{ value: `目標 ${targetP}点`, position: 'insideTopRight', fill: '#D32F2F', fontSize: 11, fontWeight: 'bold' }} />
          )}
          ...
          {/* サブ指標（細線・破線） */}
          <Line type="monotone" dataKey="X1点" stroke="#1976D2" strokeWidth={1.5}
            strokeDasharray="4 3" dot={{ r: 3 }} activeDot={{ r: 5 }} />
          <Line type="monotone" dataKey="Y点"  stroke="#388E3C" strokeWidth={1.5}
            strokeDasharray="4 3" dot={{ r: 3 }} activeDot={{ r: 5 }} />
          <Line type="monotone" dataKey="Z点"  stroke="#7B1FA2" strokeWidth={1.5}
            strokeDasharray="4 3" dot={{ r: 3 }} activeDot={{ r: 5 }} />

          {/* P点メイン（太線） */}
          <Line type="monotone" dataKey="P点" stroke="#212121" strokeWidth={3}
            dot={{ r: 6, fill: '#212121' }} activeDot={{ r: 8 }} />
      ...
      <div style={{ display: 'flex', gap: 16, justifyContent: 'center', marginTop: 4, fontSize: 11, color: '#666' }}>
        {[['A', '900点〜'], ['B', '800〜899'], ['C', '700〜799'], ['D', '〜699']].map(([r, label]) => (
          <span key={r}>
